"""
Civiora SIH Web Application Server (FastAPI + Jinja2 + Webhooks + AI Engine)
Compliant with Government_File_Verification_Workflow_Portal.md specification
"""
import uvicorn
...
# ================= STEP 1: CHOOSE ROLE ON HOME (Specification §2) =================
...
# ================= STEP 2A: CITIZEN LOGIN (Phone -> OTP -> Portal) =================
...
# ================= STEP 2B: OFFICER LOGIN (Login ID + Department + Password) =================
...
async def officer_verify_credentials(
    request: Request,
    department: str = Form(...),
    officer_login_id: str = Form(...),
    password: str = Form(...)
):
    clean_id = officer_login_id.strip().upper()
    
    # Match officer by ID or Department
    matched_officer = next((o for o in DATABASE["officers"] if o["id"].upper() == clean_id), None)
    
    if not matched_officer:
        # Fallback: check if matches officer username/department
        matched_officer = next((o for o in DATABASE["officers"] if o["department"].upper() == department.upper()), None)

    # Password check (gov@2026 or admin)
    if matched_officer and (password.strip() == "gov@2026" or password.strip() == "admin"):
        # Ensure department matches officer
        if matched_officer["department"].upper() != department.upper():
            dept_officer = next((o for o in DATABASE["officers"] if o["department"].upper() == department.upper()), matched_officer)
            target_id = dept_officer["id"]
        else:
        ...
        return templates.TemplateResponse(
            ...
            context={
                "user_role": None,
                "departments": DATABASE["departments"],
                "error": "Invalid Officer Credentials or Passcode. (Demo: gov@2026)"
            }
        )

@app.get("/login")
async def redirect_to_choose():
...
async def citizen_wizard_complete(
    ...
    digilocker_authenticated: bool = Form(True),
    has_reuploaded: bool = Form(False),
    ...
    new_file = {
        ...
        "digilocker_verified": digilocker_authenticated,
        "delay_guidance": {
            "delay_reason": "Application freshly submitted with all verified documents. In normal queue.",
            "required_action": "None at this stage.",
            "next_step": "Document verification by departmental officer desk."
        }
    }
    
    DATABASE["files"].insert(0, new_file)
    log_audit("SERVICE_SUBMISSION", f"Citizen: {applicant_name}", new_id, f"Application for {service['name']} submitted and paid (₹{service['fee']}).")
    trigger_webhook_event("NEW_SERVICE_APPLICATION", new_file, f"Application created. DigiLocker KYC verified. Payment confirmed via {payment_method}.")
    
    return RedirectResponse(url=f"/citizen?search_id={new_id}&success=true", status_code=303)

# ================= OFFICER DESK (Strict Departmental Isolation & Step Progression) =================
...
async def officer_dashboard(request: Request, officer_id: str = "OFF-103"):
    ...
    # Filter files strictly for the logged-in officer's department (e.g. MUNICIPAL, REVENUE, PENSION, EDUCATION)
...
async def officer_update_stage(
    ...
    if target_file:
        ...
        # Sequentially compute next milestone
        ...
        if calculated_stage == "DISPATCH_DIGILOCKER":
            target_file["status"] = "COMPLETED"
            target_file["delay_guidance"] = {
                "delay_reason": "Service completed successfully.",
                "required_action": "Download your official certificate / sanction order from DigiLocker.",
                "next_step": "Service Delivered."
            }
        elif calculated_stage == "INTER_DEPT_NOC":
            target_file["status"] = "IN_PROGRESS"
            target_file["delay_guidance"] = {
                "delay_reason": "Cross-departmental clearance requested.",
                "required_action": "Awaiting inter-department response.",
                "next_step": "Final sanction approval."
            }
        else:
            target_file["status"] = "IN_PROGRESS"
            target_file["delay_guidance"] = {
                "delay_reason": f"File progressed to {calculated_stage.replace('_', ' ')}.",
                "required_action": "In official processing queue.",
                "next_step": "Desk verification and milestone approval."
            }
...
async def dept_head_dashboard(request: Request):
    ...
    bottlenecks = {}
    for f in files:
        b_stage = f["prediction"]["bottleneck_stage"]
        bottlenecks[b_stage] = bottlenecks.get(b_stage, 0) + 1
...
@app.post("/admin/trigger-test-webhook")
async def trigger_test_webhook(event_type: str = Form("SIMULATED_DELAY_ALERT")):
    sample_file = DATABASE["files"][0]
    trigger_webhook_event(event_type, sample_file, "Manual test webhook dispatched from Admin Console.")
    return RedirectResponse(url="/admin?webhook_triggered=true", status_code=303)

# ================= REST API =================
@app.post("/api/predict")
async def api_predict(data: dict):
    prediction = predict_delay_and_eta(data)
    return JSONResponse(content={"status": "success", "data": prediction})
